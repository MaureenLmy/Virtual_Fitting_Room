...
public class KinectWrapper : MonoBehaviour
{
	...
	public static class Constants
	{
		...
		public static float NuiDepthHorizontalFOV = 58.5f; //水平方向视域
		public static float NuiDepthVerticalFOV = 45.6f;   //垂直方向视域
	...
	public struct NuiSkeletonData
    {
        ...
        public uint dwTrackingID;                    //这里的TrackingID 和UserIndex有什么不同
    ...
    public struct NuiSkeletonBoneRotation  //一个矩阵，一个四元组来描述骨骼的rotation
    {
        public Matrix4x4 rotationMatrix;
        public Quaternion rotationQuaternion;
    }

    public struct NuiSkeletonBoneOrientation  //目前有点难理解，
    {
    ...
    //******这里我尝试简单地把上面的set改为get,看看是不是能获取摄像机的仰角
    [DllImportAttribute(@"Kinect10.dll", EntryPoint = "NuiCameraElevationGetAngle")]
    public static extern int NuiCameraGetAngle(out long angle);//当然Get就是要传一个参数来获取返回值
    //经测验，确实是可以的，Nice!
	//*******
    ...
    //在Forum上看到的，试一下
    [DllImport("CLNUIDevice.dll", EntryPoint = "GetNUICameraColorFrameRGB24")]
    private static extern bool GetCameraColorFrameRGB24(System.IntPtr camera, System.IntPtr data, int timeout);
    // 没报错，貌似可用，不过这是用来干嘛的？
    ...
    public static Vector3 MapSkeletonPointToColorPoint(Vector3 skeletonPoint)
    {
        ...
        point.x = (int) ((fDepthX * Constants.ImageWidth) + 0.5f);
        point.y = (int) ((fDepthY * Constants.ImageHeight) + 0.5f);
        point.z = (int) (fDepthZ + 0.5f);             //颜色点要Z 值干嘛？
    ...
    {
        if (vPoint.z > float.Epsilon)  //深度不为0时才要算
        {
            pfDepthX = 0.5f + ((vPoint.x * 285.63f) / (vPoint.z * 320f));//为什么这样算？
            pfDepthY = 0.5f - ((vPoint.y * 285.63f) / (vPoint.z * 240f));
            pfDepthZ = vPoint.z * 1000f; //深度图不是也只有二维的吗，要z值干嘛
        }
        else
        {
            pfDepthX = 0f;
	...
	private static void CheckPoseComplete(ref GestureData gestureData, float timestamp, Vector3 jointPos, bool isInPose, float durationToComplete)
	{
		if(isInPose)
		{
			float timeLeft = timestamp - gestureData.timestamp; //GestureData.timestamp是初次进入要求姿势的时刻
	...
	private static void SetScreenPos(uint userId, ref GestureData gestureData, ref Vector3[] jointsPos, ref bool[] jointsTracked)
	{
		...
		if(calculateCoords)
		{
			if(gestureData.tagFloat == 0f || gestureData.userId != userId)
			{
				...
				Vector3 shoulderToElbow = elbowPos - shoulderPos; //肩膀到肘关节的长度
				Vector3 elbowToHand = handPos - elbowPos;         //肘关节到手的长度
				gestureData.tagFloat = (shoulderToElbow.magnitude + elbowToHand.magnitude);//算出当前控制人的肩膀到手的总长度
			}
	
			Vector3 shoulderToHand = handPos - shoulderPos;//肩膀到手可伸展的最长距离作为半径，
			...
			case Gestures.RaiseRightHand:
				switch(gestureData.state)
				{
					case 0:  // gesture detection
						...
						{
							SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex]);//每调用一次这个state就会向前加一
						}
						break;
				...
				switch(gestureData.state)
				{
					case 0:  // gesture detection - phase 1
						if(jointsTracked[rightHandIndex] && jointsTracked[rightElbowIndex] &&
					       (jointsPos[rightHandIndex].y - jointsPos[rightElbowIndex].y) > 0.1f && //右手举起并且在
...
//						else 
			...
			case Gestures.ZoomIn:
				...
				break;
            case Gestures.Clapping:
                switch (gestureData.state)
                {
                    case 0: //Detection phase;
                        float handsDistance=Vector3.Distance(jointsPos[leftHandIndex],jointsPos[rightHandIndex]);
                        if (jointsTracked[leftHandIndex] && jointsTracked[rightHandIndex] && jointsTracked[rightElbowIndex] && jointsTracked[leftElbowIndex]
                            && jointsPos[leftHandIndex].y>jointsPos[leftElbowIndex].y && jointsPos[rightHandIndex].y>jointsPos[rightElbowIndex].y) //能追踪到并且双手都分别别两边的肘关节高
                        {
