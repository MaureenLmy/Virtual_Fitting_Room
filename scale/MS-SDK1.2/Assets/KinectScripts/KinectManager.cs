...
public class KinectManager : MonoBehaviour
{
	...
	// List of Gestures to detect, 在Inspector 添加你想要的手势
	...
	private Matrix4x4 player1Ori, player2Ori; //描述人物朝向的矩阵
	...
	private Matrix4x4 kinectToWorld, flipMatrix; //Kienct空间坐标转换成世界坐标的矩阵
	...
	void Start()
	{
		...
		try
		{
			...
			quat.eulerAngles = new Vector3(-SensorAngle, 0.0f, 0.0f); //这里设置的感应器相当于Unity里X轴的欧拉角
		...
		if(DisplayColorMap)
		{
	        ...
	        usersClrRect = new Rect(Screen.width, Screen.height - usersClrTex.height / 2, -usersClrTex.width / 2, usersClrTex.height / 2);
                           //注意这里用到了之前没有注意到的Rect的用， 第三个和第四个参数加上负号是可以是图片左右颠倒的
                                        //后面几个为什么要除2？只显示一半？ Yes
	...
	void OnApplicationQuit()
	{
		if(KinectInitialized)
		{
			// Shutdown OpenNI    搞错了吧，MS-SDK也算OpenNI？
	...
	void ProcessSkeleton()
	{
		...
		for (int i = 0; i < KinectWrapper.Constants.NuiSkeletonCount; i++)
		{
			...
			if (skeletonData.eTrackingState == KinectWrapper.NuiSkeletonTrackingState.SkeletonTracked)
			{
				...
				Vector3 skeletonPos = kinectToWorld.MultiplyPoint3x4(skeletonData.Position); //把Kinect坐标转换成Unity里面的世界空间
                ///MultiplyPoint3x4() 是矩阵函数，是这个矩阵对坐标进行变换，MultiplyPoint is slower, but can handle projective transformations as well. 
...
//		DrawLine(aTexture, 1, 479, 639, 0, Color.yellow);
    ...
    #endregion 在Texture2D上画骨骼的方法


    #region 在Texture2D上画线的方法
